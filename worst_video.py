...
PATH_COOKIES = (
    "C:\\Users\\vince\\AppData\\Local\\Google\\Chrome\\User Data\\Default\\Network\\"
)
LINKS = {
...
def get_prev_daymonthyear_of_interest(
    month_of_interest: str, year_of_interest: str
) -> str:
    ...
    prev_year_of_interest = (
        year_of_interest
        if month_of_interest != MONTHS_NL[-1]
        else str(int(year_of_interest) - 1)
    )
...
def clean_dumpert_datestring(datestring: str) -> str:
    ...
    """
    clean_datestring = (
        datestring.split("@")[0].replace(".", "").replace("maa", "mrt").strip()
    )
...
# !! When scrolling has reached the month before, you may just interrupt the script
...
prev_dmy_of_interest = get_prev_daymonthyear_of_interest(
    month_of_interest, year_of_interest
)
...
videos = {
    video_id: {"css": video}
    for video_id, video in enumerate(driver.find_elements(By.CLASS_NAME, "css-6jb95j"))
}
...
for video_id, video in tqdm(videos.items(), desc="Retrieving video details"):
    ...
    if "vandaag" in datestring:
        date = arrow.utcnow().to("CET")
    elif "gister" in datestring:
        date = arrow.utcnow().to("CET").shift(days=-1)
    else:
        date = arrow.get(datestring, "D MMM 'YY", locale="nl-NL")
    ...
    try:
        ...
        except NoSuchElementException:  # Otherwise it is likely 0
            score = 0
    video.update(
        {
            "date": date,
...
# Filter videos for period of interest
videos = {
    k: v
    for k, v in videos.items()
    if period_of_interest_start <= v["date"] <= period_of_interest_end
}
